...
<template>
  <MobileShell eyebrow="货品去向" title="我的补货与去向" subtitle="可查看自己补货批次、当前剩余量以及相关操作日志。">
        ...
        <view v-if="batches.length" class="trace-list">
          <view v-for="item in batches" :key="item.batchId" class="trace-item">
            <view class="trace-item__main">
              ...
              <text class="trace-item__meta">
                {{ item.expiresAt ? `到期 ${formatBeijingDate(item.expiresAt)}` : "未设置保质期" }} · {{ formatBeijingDateTime(item.createdAt) }}
              </text>
            </view>
          </view>
        </view>
      ...
      <view class="vm-stack">
        ...
        <view v-if="logs.length" class="trace-list">
          <view v-for="log in logs" :key="log.id" class="trace-item">
            ...
            <text class="trace-item__meta">{{ formatBeijingDateTime(log.occurredAt) }}</text>
          </view>
        </view>
