...
test("特殊用户附近柜机保持预约与扫码并列且不再展示找柜机提示", async () => {
  ...
  assert.match(source, /可提前预约保留物资，也可到柜扫码直接领取/);
  assert.match(source, /提前预约 · 保留物资/);
  assert.match(source, /到柜扫码 · 直接领取/);
...
test("特殊用户柜机卡片只渲染有限预览并提供完整物资抽屉", async () => {
  ...
  assert.match(source, /查看全部 \{\{ entry\.visibleGoods\.length \}\} 种物资/);
