...
本说明面向当前实例管理员，说明领取设置、一次性验证码和 App 预约取货的日常操作。外部服务密钥、服务器路径和真实支付资料不写入本说明。

## 1. 先使用“示例设置”

后台“系统设置”页的首个入口是“示例设置”。日常操作只需要确认下列三项：

| 项目 | 推荐值 | 作用 |
| --- | --- | --- |
| 预约取货 | 开启 | 用户必须先预约再取货；新的领取流程不创建支付单，也不需要填写支付配置。 |
| 领取差异的额度归属 | 自动 | 柜机实际领取数量与预约不一致时，有预约按预约日计入额度，无预约按实际领取日计入额度。 |
| App 登录验证方式 | 手动设置验证码 | 实例管理员为已启用人员单独签发 6 位、短期、一次性的登录验证码，不发送短信。 |

关闭“预约取货”后，系统进入即时领取流程，并显示支付相关设置；这不修改历史订单，也不等同于启用真实支付。

保存设置后，以页面提示为准重启 API 服务；未重启时，标有“重启后生效”的设置不会用于新流程。

下图为本地隔离演练的后台设置页。页面只保留日常领取、额度归属、App 登录和运行环境选项；服务地址、密钥和发布参数不会出现在实例管理员的日常操作页。
...
“运行环境”只能从“开发环境”“测试环境”“生产环境”中选择，不能任意输入；它不是物资、预约或人员模板，也不需要匹配模板。上线后的“生产环境”由发布流程锁定，后台仅供确认，不能在线修改。

系统只有“模拟服务”和“实机服务”两类数据运行方式。设置页只展示日常领取、登录和运行环境设置；服务地址、密钥、存储位置、支付通道与柜机对接资料由服务管理员按发布资料维护，不在实例管理员日常页面显示。

## 3. 一次性验证码登录 App

一次性验证码只用于当前实例内、已启用人员的 App / 小程序登录或本人密码重置，不用于电脑后台 `admin` 账号登录。

如果知道当前 `admin` 后台密码，先正常登录，再在侧栏“修改密码”中输入当前密码和两次新密码；只有验证通过才会更新密码。唯一 `admin` 允许使用 6 位新密码，其他后台账号仍保持至少 8 位。不要把 App 一次性验证码用于后台登录；忘记后台密码时按[发布与公网部署验证流程](发布与公网部署验证流程.md)中的恢复流程处理。
...
后台和 App 登录页均以“模拟服务”或当前服务状态标识运行方式；该标识不是操作步骤。App 使用手动验证码时明确提示向实例管理员获取一次性验证码，验证码输入框限制为 6 位，且不显示“获取验证码”按钮。

后台登录页：

![公网后台登录页](assets/backoffice-login-1440x900-20260728-c482428.png)

App 登录页以 390 CSS px × 844 CSS px 手机视口采集（PNG 为 DPR 2 的 780×1688）：

![App 手动验证码登录页（390×844）](assets/app-login-manual-390x844-20260728-c482428.png)

## 6. App：从一次性验证码到预约的完整流程

以下四张图来自 2026-07-28 的本地隔离演练。演练使用实际构建的后台、API 和 H5 页面，通过 `127.0.0.1` 回环运行；人员、验证码和会话均为本次临时生成，运行数据在结束后清理。柜机、支付、地图、模型服务均为模拟实现，未操作真实柜机，也不替代当前公网实例的管理员登录记录。
...
登录页与本地隔离演练截图已经具备。认证后的公网截图只能在当前实例管理员的安全登录会话中补齐，必须遮住手机号和验证码。以下清单是最终上线前的核对项，不得用本地演练替代：
...
| App 没有“获取验证码”按钮 | App 登录验证方式是否为“手动设置验证码”。 | 这是预期界面；向实例管理员获取一次性验证码，不发送短信。 |
| 预约流程没有支付设置或支付步骤 | “预约取货”是否开启。 | 这是预约制的预期行为；不要为了继续预约而填写支付配置。 |
| 同一验证码第二次登录被拒绝 | 后台一次性验证码记录是否显示“已使用”。 | 这是单次消费保护；重新签发新码，旧码不能恢复。 |

## 9. 服务设置说明

短信、地图、柜机平台、模型服务和非预约支付属于服务设置。日常操作先使用“示例设置”；其余服务项只由已授权人员维护，敏感内容始终隐藏。地图服务未启用时，可以先手工录入坐标，再联系服务管理员完成设置。服务商提供的资料要求、真实环境切换和部署步骤应按以下文档执行：
