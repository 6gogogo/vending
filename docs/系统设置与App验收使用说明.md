...
下图为本地隔离演练的后台设置页。页面只保留日常领取、额度归属，以及当前演练所需的 App 登录选项。
...
下列匿名登录页截图来自本地隔离演练；状态标识不会遮挡按钮或输入框。App 使用手动验证码时明确提示向实例管理员获取一次性验证码，验证码输入框限制为 6 位，且不显示“获取验证码”按钮。截图未输入账号、密码、手机号或验证码。上线后仍须按第 7 节在公网重复匿名页与认证后流程核对。
...
| 系统设置：示例设置 | 预约取货、领取差异额度归属可见且布局完整；四种额度归属选项可读。当前演练实例显示 App 登录验证方式时，确认其为手动设置验证码。 |
