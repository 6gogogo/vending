...
以下截图来自 2026-07-28 的公网 HTTPS 匿名会话，发布版本为 `c482428`。采集过程未登录、未填写手机号或验证码；两端均显示业务中性的“公益服务模拟实例”标识，而不再显示旧的“验收模拟实例”文案。

后台登录页：

![公网后台登录页](assets/backoffice-login-1440x900-20260728-c482428.png)

App 登录页以 390×844 手机视口采集。人工码模式明确提示向实例管理员获取一次性验证码、输入框限制为 6 位，且不显示“获取验证码”按钮：

![App 手动验证码登录页（390×844）](assets/app-login-manual-390x844-20260728-c482428.png)

## 6. 本地隔离演练：人工码到预约的完整 UI 流程

以下四张图来自 2026-07-28 的本地隔离演练。演练使用实际构建的后台、API 和 H5 页面，通过 `127.0.0.1` 回环运行；人员、人工码和会话均为本次临时生成，运行数据在结束后清理。柜机、支付、地图、模型服务均为模拟实现，未操作真实柜机，也不替代当前公网实例的管理员认证验收。

演练通过的顺序是：后台保存预约规则 → 为已启用测试人员签发一次性人工码 → App 勾选免责声明并登录 → 选择物资、创建预约 → 确认“当前预约” → 同一人工码重放被 API 以 `401` 拒绝 → 后台记录显示“已使用”。图片中不含手机号、验证码或后台密码。

1. 手动验证码入口：页面提示向实例管理员获取一次性验证码；输入框为 6 位，且没有短信“获取验证码”按钮。

![本地隔离演练：App 人工码入口（390×844）](assets/app-local-isolated-manual-login-390x844-20260728.png)

2. 登录成功后进入用户身份首页，可以看到当天的领取资格和附近柜机入口。

![本地隔离演练：App 登录成功（390×844）](assets/app-local-isolated-login-success-390x844-20260728.png)

3. 在柜机详情选择物资并提交后，会显示“预约成功”；预约流程不显示支付步骤。

![本地隔离演练：预约成功确认（390×844）](assets/app-local-isolated-reservation-success-390x844-20260728.png)

4. 关闭确认框后，页面保留“当前预约”、保留时间和“用预约开柜”前的状态。演练在此停止，不执行开柜。

![本地隔离演练：当前预约（390×844）](assets/app-local-isolated-reservation-active-390x844-20260728.png)

## 7. 公网管理员最终截图清单

当前匿名入口截图与本地隔离演练已经具备。认证后的公网截图只能在当前实例管理员的安全登录会话中补齐，必须遮住手机号和验证码。由于公网 `admin` 首次密码维护尚未成功完成，以下清单仍是最终上线前的待验收项，不得用本地演练替代：
