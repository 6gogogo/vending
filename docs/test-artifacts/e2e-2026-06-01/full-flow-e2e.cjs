...
(async () => {
  ...
  assertStep("小程序提交受助用户注册申请", createdApplication.status === "pending", createdApplication.id);

  const publicLookup = await req("GET", `/registration-applications/by-phone?phone=${specialPhone}`);
  assertStep("公开手机号查询只返回审核状态", publicLookup.state === "pending");

  const pendingLogin = await req("POST", "/auth/app-login", { phone: specialPhone, code: registerCode });
  assertStep("待审核用户登录进入审核状态", pendingLogin.state === "pending_review");

  try {
    await req("POST", "/auth/request-code", { phone: specialPhone, scene: "app-login" });
  ...
  const appSession = await req("POST", "/auth/app-login", { phone: specialPhone, code: registerCode });
  ...
  assertStep("小程序额度显示当前策略", (quota.remainingByGoods?.[goods.goodsId] ?? 0) >= 2);
  ...
  assertStep("小程序可提前预约货品", reservation.status === "active", reservation.id);
  ...
  await req("POST", "/cabinet-events/callbacks/settlement", {
    ...
    deviceCode: device.deviceCode,
    amount: 0,
    notifyUrl: `${base}/cabinet-events/callbacks/payment-success`,
  ...
  const merchantCode = await codeFor(merchantPhone, "app-login");
  const merchantSession = await req("POST", "/auth/app-login", { phone: merchantPhone, code: merchantCode });
